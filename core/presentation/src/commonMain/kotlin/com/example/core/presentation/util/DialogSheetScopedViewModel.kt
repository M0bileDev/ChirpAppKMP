...
fun DialogSheetScopedViewModel(
    ...
    val parentStoreOwner = LocalViewModelStoreOwner.current ?: error("No parent store owner found")

    val registry = koinViewModel<ScopedStoreRegistryViewModel>(
        viewModelStoreOwner = parentStoreOwner
    )
    ...
    owner?.let { dialogSheetOwner ->
        CompositionLocalProvider(LocalViewModelStoreOwner provides dialogSheetOwner) {
            content()
        }
    }
}